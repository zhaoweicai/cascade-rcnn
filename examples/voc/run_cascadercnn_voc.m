...
cocoDir = '/your/path/to/coco'; % your COCO API directory
addpath(genpath([cocoDir '/MatlabAPI']));

%% experimental setup %%
% network
root_dir = './res101-9s-600-rfcn-cascade-pretrained/';
binary_file = [root_dir 'cascadercnn_voc_iter_90000.caffemodel'];
...
% dataset
dataDir = '/your/voc/path';  % your VOC directory has dataDir/VOCdevkit
dataYear = '2007'; dataSplit='test';
annFile=sprintf('VOC%s_%s_cocoformat.json',dataYear, dataSplit);
if (~exist(annFile))
  CocoUtils.convertPascalGt(dataDir, dataYear, dataSplit, annFile);
end
...
det_thr = 0.1; % threoshold
max_per_img = -1; % max number of detections
pNms.type = 'maxg'; pNms.overlap = 0.3; pNms.ovrDnm = 'union'; % NMS

% saveing ID and evaluation log
comp_id = 'tmp'; % specify a unique ID if you want to archive the results
log_id = '90k';
eval_log = sprintf('%svoc_eval_%s_%s.txt',root_dir,log_id,dataSplit);

% image pre-processing
shortSize = 600; longSize = 1000;
...
for kk = 1:nImg
  ...
  img = imread(sprintf('%s/VOCdevkit/VOC%s/JPEGImages/%s',dataDir,...
      dataYear,imgInfo.file_name));
  orgImg = img;
...
for nn = 1:num_outputs
  % saving
  resFile = sprintf('%s%s_%s_%s_results.json',save_dir,comp_id,dataSplit,output_names{nn});
