      ...
      if (Caffe::mode() == Caffe::GPU) {
        ...
        if (this->output_labels_) {
          for (int nn = 0; nn < batch->labels_.size(); nn++) {
            batch->labels_[nn]->data().get()->async_gpu_push(stream);
          }
        }
...
void BasePrefetchingDataLayer<Dtype>::Forward_cpu(
  ...
  if (this->output_labels_) {
    for (int nn = 0; nn < prefetch_current_->labels_.size(); nn++) {
      // Reshape to loaded labels.
      top[nn+1]->ReshapeLike(*prefetch_current_->labels_[nn]);
      top[nn+1]->set_cpu_data(prefetch_current_->labels_[nn]->mutable_cpu_data());
    }
  }
}
