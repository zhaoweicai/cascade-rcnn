...
void Solver<Dtype>::Test(const int test_net_id) {
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    if (i == 0) {
      for (int j = 0; j < result.size(); ++j) {
        ...
        for (int k = 0; k < result[j]->count(); ++k) {
          if (result_vec[k] < 0) {
            test_score.push_back(Dtype(0));
            ignore_iters.push_back(1);
          } else {
            test_score.push_back(result_vec[k]);
            ignore_iters.push_back(0);
          }
      ...
      for (int j = 0; j < result.size(); ++j) {
        ...
        for (int k = 0; k < result[j]->count(); ++k) {
          if (result_vec[k] < 0) {
            ignore_iters[idx]++; idx++;
          } else {
            test_score[idx] += result_vec[k]; idx++;
          }
        }
      }
    }
  ...
  for (int i = 0; i < test_score.size(); ++i) {
    ...
    ostringstream loss_msg_stream;
    const Dtype mean_score = test_score[i] / Dtype(param_.test_iter(test_net_id)-ignore_iters[i]);
    if (loss_weight) {
      loss_msg_stream << " (* " << loss_weight
...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    for (int j = 0; j < result.size(); ++j) {
      CHECK_EQ(result[j]->channels(), 5);
      const Dtype* result_vec = result[j]->cpu_data();
      int num_det = result[j]->num();
